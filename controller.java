...
public Label msg;
 ...
 @FXML
 public TextField srch;
 @FXML
 public Button b2;
 @FXML
 private TextField atno1;
 int flag=0;
 
 
 
 public void src(ActionEvent event) throws Exception {
	
	 String res=srch.getText();
	
	 for(int i=0;i<ename.length;i++) {
		
		 if(ename[i].equalsIgnoreCase(res)) {
				flag++;
				 			msg.setText("Name:-"+ename[i] + "\n" +"Symbol:-"+symbol[i] + "\n"+ "Group:-"+ group[i]+ "\n"+ "Period:-"+ period[i]
							 	+ "\n"+ "Atomic Number:-"+ atno[i] + "\n"+ "Atomic Weight:-" + atwt[i]+ "\n"+ "Type:-" + type[i]+ "\n"+
							 		"Protons:-"+ pro[i]	+"\n"+"Electrons:-"+ elec[i]+"\n" + "Neutrons:-"+ neu[i]);
